...
def convert_image(image_source: Path, image_destination: Path) -> bool:
    ...
    this_factory = get_image_factory(
        image_source.suffix, image_destination.suffix)
...
def main() -> None:
    ...
    result = convert_image(Path("path/to/image1.png"),
                           Path("path/to/image2.jpeg"))
    if not result:
        print("the PNG to JPEG conversion failed")
    else:
        print("the PNG to JPEG conversion succeeded")

    result = convert_image(Path("path/to/image1.jpg"),
